...
public final class ConfigScreen extends Screen {
        ...
        if (control.kind() == ConfigControlKind.TEXT_FIELD) {
            ...
            if (control.labelBounds() != null) {
                ...
                TextWidget label = new TextWidget(labelBounds.x(), labelBounds.y(), labelBounds.width(), labelBounds.height(),
                        withColor(toText(control.label()), control.color()), textRenderer);
            ...
            Text message = withColor(toText(control.label()), control.color());
            int textWidth = Math.max(1, Math.min(bounds.width(), textRenderer.getWidth(message)));
            int textX = control.alignment() == ConfigControlView.TextAlignment.CENTER
                    ? bounds.x() + (bounds.width() - textWidth) / 2
                    : bounds.x();
            TextWidget text = new TextWidget(textX, bounds.y(), textWidth, bounds.height(), message, textRenderer);
            widget = text;
        }
    ...
    private void refreshDynamicControls() {
        ...
        for (ConfigControlView control : pageView.controls()) {
            ...
            if (widget instanceof TextWidget text) {
                text.setMessage(withColor(toText(control.label()), control.color()));
            }
        }
    }
    ...
    private static Text withColor(Text text, int color) {
        return text.copy().styled(style -> style.withColor(color & 0xFFFFFF));
    }
