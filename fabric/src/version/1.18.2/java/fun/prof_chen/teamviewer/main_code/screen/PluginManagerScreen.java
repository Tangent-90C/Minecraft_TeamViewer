...
public final class PluginManagerScreen extends Screen {
    ...
    public PluginManagerScreen(Screen parent, PluginManagerUiController controller) {
        super(new TranslatableText("screen.mc_teamviewer.integration_plugin.manager.title"));
        this.parent = parent;
    ...
    public void render(MatrixStack context, int mouseX, int mouseY, float delta) {
        ...
        if (tooltip != null) renderTooltip(context, List.of(toText(tooltip)), mouseX, mouseY);
    }

    private void drawManager(MatrixStack context, int mouseX, int mouseY) {
        fill(context, view.frame(), FRAME);
        border(context, view.frame(), BORDER);
        fill(context, view.header(), PANEL_ALT);
        DrawableHelper.fill(context, view.header().x(), view.header().y() + view.header().height() - 1,
                view.header().x() + view.header().width(), view.header().y() + view.header().height(), BORDER);
        drawClipped(context, new TranslatableText("screen.mc_teamviewer.integration_plugin.manager.title"),
    ...
    private void drawList(MatrixStack context, int mouseX, int mouseY) {
        fill(context, view.listBounds(), PANEL);
        if (!view.compact()) {
            DrawableHelper.fill(context, view.listBounds().x() + view.listBounds().width(), view.listBounds().y(),
                    view.listBounds().x() + view.listBounds().width() + 1,
                    view.listBounds().y() + view.listBounds().height(), BORDER);
        }
        enableScissor(view.listBounds());
    ...
    private void drawListItem(MatrixStack context, ListItemView item, int mouseX, int mouseY) {
        boolean hovered = contains(item.bounds(), mouseX, mouseY);
        fill(context, item.bounds(), item.selected() ? SELECTED : hovered ? HOVERED : PANEL);
        if (item.selected()) {
            DrawableHelper.fill(context, item.bounds().x(), item.bounds().y(), item.bounds().x() + 2,
                    item.bounds().y() + item.bounds().height(), ACCENT);
        }
        DrawableHelper.fill(context, item.bounds().x(), item.bounds().y() + item.bounds().height() - 1,
                item.bounds().x() + item.bounds().width(), item.bounds().y() + item.bounds().height(), BORDER);
        DrawableHelper.fill(context, item.bounds().x() + 8, item.bounds().y() + 9,
    ...
    private void drawDetail(MatrixStack context, int mouseX, int mouseY) {
        ...
        DrawableHelper.fill(context, titleX, detail.bounds().y() + 48, titleX + 4, detail.bounds().y() + 52,
                detail.statusColor());
        ...
        DrawableHelper.fill(context, detail.bounds().x(), detail.bounds().y() + 66,
                detail.bounds().x() + detail.bounds().width(), detail.bounds().y() + 67, BORDER);

        enableScissor(detail.contentBounds());
    ...
    private void drawLine(MatrixStack context, LineView line) {
        fill(context, line.bounds(), PANEL);
        DrawableHelper.fill(context, line.bounds().x(), line.bounds().y() + line.bounds().height() - 1,
    ...
    private void drawSetting(MatrixStack context, SettingView setting, int mouseX, int mouseY) {
        fill(context, setting.bounds(), PANEL);
        DrawableHelper.fill(context, setting.bounds().x(), setting.bounds().y() + setting.bounds().height() - 1,
    ...
    private void drawAction(MatrixStack context, ActionView action, int mouseX, int mouseY) {
    ...
    private void drawSwitch(MatrixStack context, UiRect bounds, boolean checked, boolean active) {
        ...
        DrawableHelper.fill(context, knobX, bounds.y() + 3, knobX + 7, bounds.y() + bounds.height() - 3,
                active ? TEXT : MUTED);
    }

    private void drawMessage(MatrixStack context, MessageView message) {
        fill(context, message.bounds(), PANEL);
        DrawableHelper.fill(context, message.bounds().x(), message.bounds().y(),
    ...
    private void drawDialog(MatrixStack context, DialogView dialog, int mouseX, int mouseY) {
        DrawableHelper.fill(context, view.frame().x(), view.frame().y(),
    ...
    private void drawClipped(MatrixStack context, Text text, UiRect bounds, int color, boolean shadow) {
    ...
    private void drawCentered(MatrixStack context, UiText text, int centerX, int y, int color) {
        Text value = toText(text);
        textRenderer.draw(context, value, centerX - textRenderer.getWidth(value) / 2, y, color);
    }

    private static void fill(MatrixStack context, UiRect rect, int color) {
        DrawableHelper.fill(context, rect.x(), rect.y(), rect.x() + rect.width(), rect.y() + rect.height(), color);
    }

    private static void border(MatrixStack context, UiRect rect, int color) {
        DrawableHelper.fill(context, rect.x(), rect.y(), rect.x() + rect.width(), rect.y() + 1, color);
        DrawableHelper.fill(context, rect.x(), rect.y() + rect.height() - 1,
                rect.x() + rect.width(), rect.y() + rect.height(), color);
        DrawableHelper.fill(context, rect.x(), rect.y(), rect.x() + 1, rect.y() + rect.height(), color);
        DrawableHelper.fill(context, rect.x() + rect.width() - 1, rect.y(),
                rect.x() + rect.width(), rect.y() + rect.height(), color);
    }
    ...
    private void enableScissor(UiRect bounds) {
