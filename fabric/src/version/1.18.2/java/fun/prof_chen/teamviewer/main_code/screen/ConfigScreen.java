...
public final class ConfigScreen extends Screen {
    ...
    private void addControl(ConfigControlView control) {
        ...
        if (control.kind() == ConfigControlKind.TEXT_FIELD) {
            ...
            widget = new ButtonWidget(
                    bounds.x(), bounds.y(), bounds.width(), bounds.height(), styledText(control),
                    ignored -> activate(control.id()),
                    control.tooltip() == null
                            ? ButtonWidget.EMPTY
                            : (ignored, matrices, mouseX, mouseY) ->
                                    renderTooltip(matrices, toText(control.tooltip()), mouseX, mouseY));
        } else {
            return;
        }
    ...
    private static Text toText(UiText value) {
        ...
        if (value.translationKey() != null) {
            ...
            text = new TranslatableText(value.translationKey(), arguments);
        } else {
            text = new LiteralText(value.literal() == null ? "" : value.literal());
        }
