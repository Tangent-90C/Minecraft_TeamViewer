...
public class StandaloneMultiPlayerESP implements ClientModInitializer {
	...
	public void onInitializeClient() {
		...
		sharedWaypointSyncCoordinator = new SharedWaypointSyncCoordinator(
				...
				waypointSyncGateway,
				minimapBridgeRegistry.sharedWaypointAdapters());
		...
		ClientTickEvents.END_CLIENT_TICK.register(client -> {
			...
			while (markKey.wasPressed()) {
				if (canCreateMark(client)) {
					handleQuickMarkAction(client, true);
				}
			}
	...
	private void renderESP(WorldRenderContext context) {
		...
		for (Map.Entry<UUID, Vec3d> entry : positions.entrySet()) {
			...
			String playerName = getPlayerNameByUuid(client, entry.getKey());
			TeamRelationHelper.TeamRelation localRelation = TeamRelationHelper.determineRelation(client, entry.getKey(), playerName, config);
			int boxRenderColor = resolveRenderColorWithPriority(localRelation, markedTeam, config.getBoxColor());
			int tracerRenderColor = resolveRenderColorWithPriority(localRelation, markedTeam, config.getLineColor());
	...
	private Map<UUID, RemotePlayerInfo> resolvePlayersForWorldMapBridge() {
		...
		Map<UUID, RemotePlayerInfo> mergedPlayers = new HashMap<>(remotePlayerRepository == null ? remotePlayers : remotePlayerRepository.snapshot());
		MinecraftClient client = MinecraftClient.getInstance();
		if (client.world != null) {
			for (AbstractClientPlayerEntity player : client.world.getPlayers()) {
				...
				RemotePlayerInfo localInfo = new RemotePlayerInfo(
					player.getUuid(),
					MinecraftPositionAdapter.fromVec3d(player.getPos()),
					MinecraftDimensionAdapter.toDimensionId(player.getWorld().getRegistryKey()),
	...
	private int resolveRenderColorWithPriority(TeamRelationHelper.TeamRelation localRelation, String serverTeamTag, int fallbackColor) {
		boolean preferLocal = config != null && config.isPreferLocalDataForEsp();
		
		if (preferLocal) {
			if (localRelation != null && localRelation != TeamRelationHelper.TeamRelation.UNKNOWN) {
				return switch (localRelation) {
					case FRIENDLY -> config.getFriendlyTeamColor();
					case ENEMY -> config.getEnemyTeamColor();
					case NEUTRAL -> config.getNeutralTeamColor();
					case UNKNOWN -> resolveRenderColorByTeam(serverTeamTag, fallbackColor);
				};
			}
			return resolveRenderColorByTeam(serverTeamTag, fallbackColor);
		} else {
			if (serverTeamTag != null && !serverTeamTag.isBlank()) {
				return resolveRenderColorByTeam(serverTeamTag, fallbackColor);
			}
			if (localRelation != null && localRelation != TeamRelationHelper.TeamRelation.UNKNOWN) {
				return switch (localRelation) {
	...
	private String getPlayerNameByUuid(MinecraftClient client, UUID uuid) {
		...
		RemotePlayerInfo remoteInfo = remotePlayers.get(uuid);
		if (remoteInfo != null) {
	...
	private void handleMiddleMouseDoubleClickMarking(MinecraftClient client) {
		...
		if (middlePressed && !middlePressedLastTick) {
			...
			if (now - lastMiddleClickTs <= MARK_DOUBLE_CLICK_MS) {
				lastMiddleClickTs = 0L;
				handleQuickMarkAction(client, false);
			} else {
	...
	private void handleAutoCancelWaypointOnEntityDeath(MinecraftClient client) {
		...
		for (String waypointId : toDelete) {
			SharedWaypointInfo removed = sharedWaypointRepository.remove(waypointId);
			trackedEntityWaypointLastPositions.remove(waypointId);
			sharedWaypointSyncCoordinator.deleteManagedWaypoint(waypointId);
		...
		if (networkManager != null && networkManager.isConnected()) {
			waypointSyncGateway.sendWaypointEntityDeathCancel(localPlayerId, cancelledTargetEntityIds);
		}
	}
	...
	private boolean tryCancelTargetedWaypoint(MinecraftClient client) {
		...
		SharedWaypointInfo removed = sharedWaypointRepository.remove(selectedWaypointId);
		trackedEntityWaypointLastPositions.remove(selectedWaypointId);
		sharedWaypointSyncCoordinator.deleteLocalWaypoints(localPlayerId, List.of(selectedWaypointId));
	...
	private void handleQuickMarkAction(MinecraftClient client, boolean tryCancelFirst) {
		if (client == null) {
			return;
		}
		if (tryCancelFirst && tryCancelTargetedWaypoint(client)) {
			return;
		}
	...
	private void createAndSyncQuickMark(MinecraftClient client) {
		...
		if (!overflowQuickWaypointIds.isEmpty()) {
			sharedWaypointSyncCoordinator.deleteLocalWaypoints(ownerId, overflowQuickWaypointIds);
		}
		...
		sharedWaypointSyncCoordinator.upsertLocalWaypoints(
				ownerId,
				Map.of(waypointId, WaypointSyncPayload.quickMark(waypoint, config.getWaypointTimeoutSeconds(), maxQuickMarkCount)));
	...
	private Vec3d resolveWaypointWorldPosition(MinecraftClient client, SharedWaypointInfo waypoint, String currentDimension) {
		...
		Position3D remoteEntity = waypointSyncGateway == null ? null : waypointSyncGateway.getRemoteEntityPosition(targetEntityId, currentDimension);
		Vec3d remoteEntityPos = MinecraftPositionAdapter.toVec3d(remoteEntity);
		if (remoteEntityPos != null) {
			trackedEntityWaypointLastPositions.put(waypoint.waypointId(), remoteEntityPos);
			return remoteEntityPos;
		}

		if (isPlayerTarget(waypoint) && waypointSyncGateway != null) {
			Position3D remotePlayer = waypointSyncGateway.getRemotePlayerPosition(targetEntityId, waypoint.targetEntityName(), currentDimension);
			Vec3d remotePlayerPos = MinecraftPositionAdapter.toVec3d(remotePlayer);
