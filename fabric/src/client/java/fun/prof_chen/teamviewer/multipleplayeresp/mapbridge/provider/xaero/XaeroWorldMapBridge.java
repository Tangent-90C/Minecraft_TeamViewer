...
public final class XaeroWorldMapBridge {
	...
	public static boolean isAvailable() {
		return FabricLoader.getInstance().isModLoaded(XAERO_MOD_ID) && !disabled;
	}
	...
	private static Iterator<RemotePlayerInfo> buildTrackedPlayerIterator() {
		...
		String currentDimension = client.world != null
				? MinecraftDimensionAdapter.toDimensionId(client.world.getRegistryKey())
				: null;
	...
	private static final class TrackedPlayerReaderInvocation implements InvocationHandler {
		...
		public Object invoke(Object proxy, Method method, Object[] args) {
			...
			return switch (method.getName()) {
				...
				case "getDimension" -> MinecraftDimensionAdapter.toRegistryKey(info.dimension(), World.OVERWORLD);
				case "getColor" -> resolvePlayerColor(info);
				case "getColorIndex" -> resolvePlayerColor(info);
				default -> null;
			};
		}

		private static int resolvePlayerColor(RemotePlayerInfo info) {
			MinecraftClient client = MinecraftClient.getInstance();
			Config config = StandaloneMultiPlayerESP.getConfig();
			TeamRelationHelper.TeamRelation relation = TeamRelationHelper.determineRelation(client, info.uuid(), info.name(), config);
			return resolveTeamColor(relation, config);
		}
