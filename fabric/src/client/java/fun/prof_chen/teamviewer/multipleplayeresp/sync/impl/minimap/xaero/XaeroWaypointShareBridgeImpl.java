...
public final class XaeroWaypointShareBridgeImpl {
	...
	public static boolean isAvailable() {
		return FabricLoader.getInstance().isModLoaded(XAERO_MINIMAP_MOD_ID);
	}
	...
	public static void clear() {
		...
		try {
			...
			if (removeAllSharedWaypoints(currentWaypointSet)) {
				saveWaypointWorld(minimapSession, currentWorld);
			}
		} catch (Exception e) {
			LOGGER.debug("Failed to clear Xaero shared waypoint state: {}", e.getMessage());
		}
	}
	...
	public static void tick(WaypointSyncGateway networkGateway, Map<String, SharedWaypointInfo> remoteWaypoints, boolean espEnabled, SharedWaypointBridgeConfig config) {
		...
		latestRemoteWaypoints.clear();
		if (remoteWaypoints != null && !remoteWaypoints.isEmpty()) {
			latestRemoteWaypoints.putAll(remoteWaypoints);
		}

		long now = System.currentTimeMillis();
		if (!networkGateway.isConnected()) {
			remoteReconcileRequired = true;
		}

		if (networkGateway.isConnected() && now - lastLocalScanMs >= LOCAL_SCAN_INTERVAL_MS) {
			lastLocalScanMs = now;
			scanAndSyncLocalWaypoints(networkGateway, config);
		}

		if (networkGateway.isConnected() && now - lastRemoteSyncMs >= REMOTE_SYNC_INTERVAL_MS) {
			lastRemoteSyncMs = now;
			syncRemoteWaypoints(networkGateway, config);
		}
	}
	...
	private static void scanAndSyncLocalWaypoints(WaypointSyncGateway networkGateway, SharedWaypointBridgeConfig config) {
		try {
			...
			syncDeletedSharedWaypointsFromMinimap(networkGateway, ownerId, dimension, config);
			...
			Map<String, WaypointSyncPayload> toUpload = new HashMap<>();
			...
			for (Map.Entry<String, SharedWaypointInfo> entry : currentLocal.entrySet()) {
				if (!knownLocalWaypoints.containsKey(entry.getKey())) {
					...
					toUpload.put(entry.getKey(), WaypointSyncPayload.manual(waypoint, manualTtl));
				}
			}
			...
			if (!toUpload.isEmpty()) {
				networkGateway.sendWaypointUpserts(ownerId, toUpload);
			}
			if (!toDelete.isEmpty()) {
				networkGateway.sendWaypointDeletes(ownerId, toDelete);
			}
	...
	private static void syncDeletedSharedWaypointsFromMinimap(
		...
		try {
			...
			for (String waypointId : new ArrayList<>(appliedRemoteWaypointNames.keySet())) {
				...
				Object demotedObject = findDemotedSharedWaypointObject(currentWaypointSet, waypoint, currentDimension, networkGateway);
			...
			if (!deletedIds.isEmpty()) {
				...
				networkGateway.sendWaypointDeletes(localPlayerId, deletedIds);
			}
		} catch (Exception e) {
	...
	private static void syncRemoteWaypoints(WaypointSyncGateway networkGateway, SharedWaypointBridgeConfig config) {
		try {
			...
			for (Map.Entry<String, SharedWaypointInfo> entry : latestRemoteWaypoints.entrySet()) {
				...
				Vec3d trackedPos = resolveWaypointSyncPosition(waypoint, currentDimension, networkGateway);
	...
	private static Vec3d resolveWaypointSyncPosition(SharedWaypointInfo waypoint, String currentDimension, WaypointSyncGateway networkGateway) {
		if (waypoint == null) {
			return null;
		}
		...
		if (networkGateway != null) {
			Position3D remoteEntity = networkGateway.getRemoteEntityPosition(targetEntityId, currentDimension);
			Vec3d remotePos = MinecraftPositionAdapter.toVec3d(remoteEntity);
			if (remotePos != null) {
			...
			if (isPlayerTarget(waypoint)) {
				Position3D remotePlayer = networkGateway.getRemotePlayerPosition(targetEntityId, waypoint.targetEntityName(), currentDimension);
	...
	private static Object findDemotedSharedWaypointObject(Object waypointSet, SharedWaypointInfo waypoint, String currentDimension, WaypointSyncGateway networkGateway) {
		...
		Vec3d resolvedPos = resolveWaypointSyncPosition(waypoint, currentDimension, networkGateway);
