...
public class PlayerESPWaypointConfigScreen extends Screen {
    ...
    private TextFieldWidget friendlyPrefixesField;
    private TextFieldWidget enemyPrefixesField;
    ...
    private ButtonWidget enablePlayerWaypointMarkerButton;
    private ButtonWidget showFriendlyPlayerMarkerButton;
    private ButtonWidget showEnemyPlayerMarkerButton;
    private ButtonWidget showNeutralPlayerMarkerButton;
    ...
    protected void init() {
        ...
        int playerMarkerY = getNextButtonY();
        this.enablePlayerWaypointMarkerButton = ButtonWidget.builder(
            Text.translatable("screen.multipleplayeresp.config.enable_player_waypoint_marker"),
            button -> toggleEnablePlayerWaypointMarker()
        ).dimensions(leftX, playerMarkerY, COMPONENT_WIDTH, COMPONENT_HEIGHT).build();
        this.addDrawableChild(this.enablePlayerWaypointMarkerButton);

        int playerMarkerFilterY = getNextButtonY();
        this.showFriendlyPlayerMarkerButton = ButtonWidget.builder(
            Text.translatable("screen.multipleplayeresp.config.show_friendly_player_marker"),
            button -> toggleShowFriendlyPlayerMarker()
        ).dimensions(leftX, playerMarkerFilterY, COMPONENT_WIDTH, COMPONENT_HEIGHT).build();
        this.addDrawableChild(this.showFriendlyPlayerMarkerButton);

        this.showEnemyPlayerMarkerButton = ButtonWidget.builder(
            Text.translatable("screen.multipleplayeresp.config.show_enemy_player_marker"),
            button -> toggleShowEnemyPlayerMarker()
        ).dimensions(rightX, playerMarkerFilterY, COMPONENT_WIDTH, COMPONENT_HEIGHT).build();
        this.addDrawableChild(this.showEnemyPlayerMarkerButton);

        int neutralMarkerY = getNextButtonY();
        this.showNeutralPlayerMarkerButton = ButtonWidget.builder(
            Text.translatable("screen.multipleplayeresp.config.show_neutral_player_marker"),
            button -> toggleShowNeutralPlayerMarker()
        ).dimensions(leftX, neutralMarkerY, COMPONENT_WIDTH, COMPONENT_HEIGHT).build();
        this.addDrawableChild(this.showNeutralPlayerMarkerButton);

        int friendlyPrefixY = getNextY();
        this.friendlyPrefixesField = new TextFieldWidget(
            this.textRenderer,
            leftX,
            friendlyPrefixY,
            COMPONENT_WIDTH,
            COMPONENT_HEIGHT,
            Text.translatable("screen.multipleplayeresp.config.friendly_prefixes")
        );
        this.friendlyPrefixesField.setText(StandaloneMultiPlayerESP.getConfig().getFriendlyPrefixes());
        this.friendlyPrefixesField.setMaxLength(256);
        this.friendlyPrefixesField.setPlaceholder(Text.translatable("screen.multipleplayeresp.config.friendly_prefixes_hint"));
        this.addDrawableChild(this.friendlyPrefixesField);

        this.addDrawableChild(
            new TextWidget(leftX, friendlyPrefixY - LABEL_SPACING, COMPONENT_WIDTH, 12,
                Text.translatable("screen.multipleplayeresp.config.friendly_prefixes"), this.textRenderer)
                .alignLeft()
                .setTextColor(0xFFFFFF)
        );

        this.enemyPrefixesField = new TextFieldWidget(
            this.textRenderer,
            rightX,
            friendlyPrefixY,
            COMPONENT_WIDTH,
            COMPONENT_HEIGHT,
            Text.translatable("screen.multipleplayeresp.config.enemy_prefixes")
        );
        this.enemyPrefixesField.setText(StandaloneMultiPlayerESP.getConfig().getEnemyPrefixes());
        this.enemyPrefixesField.setMaxLength(256);
        this.enemyPrefixesField.setPlaceholder(Text.translatable("screen.multipleplayeresp.config.enemy_prefixes_hint"));
        this.addDrawableChild(this.enemyPrefixesField);

        this.addDrawableChild(
            new TextWidget(rightX, friendlyPrefixY - LABEL_SPACING, COMPONENT_WIDTH, 12,
                Text.translatable("screen.multipleplayeresp.config.enemy_prefixes"), this.textRenderer)
                .alignLeft()
                .setTextColor(0xFFFFFF)
        );
        ...
        updateEnablePlayerWaypointMarkerButton();
        updateShowFriendlyPlayerMarkerButton();
        updateShowEnemyPlayerMarkerButton();
        updateShowNeutralPlayerMarkerButton();
    }
    ...
    private void toggleEnablePlayerWaypointMarker() {
        Config config = StandaloneMultiPlayerESP.getConfig();
        boolean currentStatus = config.isEnablePlayerWaypointMarker();
        boolean newStatus = !currentStatus;
        config.setEnablePlayerWaypointMarker(newStatus);
        config.setShowFriendlyPlayerMarkers(newStatus);
        config.setShowEnemyPlayerMarkers(newStatus);
        config.setShowNeutralPlayerMarkers(newStatus);
        updateEnablePlayerWaypointMarkerButton();
        updateShowFriendlyPlayerMarkerButton();
        updateShowEnemyPlayerMarkerButton();
        updateShowNeutralPlayerMarkerButton();
    ...
    private void updateEnablePlayerWaypointMarkerButton() {
        if (this.enablePlayerWaypointMarkerButton != null) {
            boolean isEnabled = StandaloneMultiPlayerESP.getConfig().isEnablePlayerWaypointMarker();
            String buttonText = Text.translatable("screen.multipleplayeresp.config.enable_player_waypoint_marker").getString();
            buttonText += isEnabled ? " [ON]" : " [OFF]";
            this.enablePlayerWaypointMarkerButton.setMessage(Text.of(buttonText));
        }
    }

    private void toggleShowFriendlyPlayerMarker() {
        boolean currentStatus = StandaloneMultiPlayerESP.getConfig().isShowFriendlyPlayerMarkers();
        StandaloneMultiPlayerESP.getConfig().setShowFriendlyPlayerMarkers(!currentStatus);
        updateShowFriendlyPlayerMarkerButton();
    }

    private void updateShowFriendlyPlayerMarkerButton() {
        if (this.showFriendlyPlayerMarkerButton != null) {
            boolean isEnabled = StandaloneMultiPlayerESP.getConfig().isShowFriendlyPlayerMarkers();
            String buttonText = Text.translatable("screen.multipleplayeresp.config.show_friendly_player_marker").getString();
            buttonText += isEnabled ? " [ON]" : " [OFF]";
            this.showFriendlyPlayerMarkerButton.setMessage(Text.of(buttonText));
        }
    }

    private void toggleShowEnemyPlayerMarker() {
        boolean currentStatus = StandaloneMultiPlayerESP.getConfig().isShowEnemyPlayerMarkers();
        StandaloneMultiPlayerESP.getConfig().setShowEnemyPlayerMarkers(!currentStatus);
        updateShowEnemyPlayerMarkerButton();
    }

    private void updateShowEnemyPlayerMarkerButton() {
        if (this.showEnemyPlayerMarkerButton != null) {
            boolean isEnabled = StandaloneMultiPlayerESP.getConfig().isShowEnemyPlayerMarkers();
            String buttonText = Text.translatable("screen.multipleplayeresp.config.show_enemy_player_marker").getString();
            buttonText += isEnabled ? " [ON]" : " [OFF]";
            this.showEnemyPlayerMarkerButton.setMessage(Text.of(buttonText));
        }
    }

    private void toggleShowNeutralPlayerMarker() {
        boolean currentStatus = StandaloneMultiPlayerESP.getConfig().isShowNeutralPlayerMarkers();
        StandaloneMultiPlayerESP.getConfig().setShowNeutralPlayerMarkers(!currentStatus);
        updateShowNeutralPlayerMarkerButton();
    }

    private void updateShowNeutralPlayerMarkerButton() {
        if (this.showNeutralPlayerMarkerButton != null) {
            boolean isEnabled = StandaloneMultiPlayerESP.getConfig().isShowNeutralPlayerMarkers();
            String buttonText = Text.translatable("screen.multipleplayeresp.config.show_neutral_player_marker").getString();
            buttonText += isEnabled ? " [ON]" : " [OFF]";
            this.showNeutralPlayerMarkerButton.setMessage(Text.of(buttonText));
        }
    }
    ...
    private void applyFieldValues() {
        ...
        if (this.friendlyPrefixesField != null) {
            String friendlyPrefixes = this.friendlyPrefixesField.getText().trim();
            StandaloneMultiPlayerESP.getConfig().setFriendlyPrefixes(friendlyPrefixes);
        }

        if (this.enemyPrefixesField != null) {
            String enemyPrefixes = this.enemyPrefixesField.getText().trim();
            StandaloneMultiPlayerESP.getConfig().setEnemyPrefixes(enemyPrefixes);
        }
    }
