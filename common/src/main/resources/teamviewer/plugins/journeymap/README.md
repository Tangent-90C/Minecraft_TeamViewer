...
The manifest selects `fabric-1.21.8.lua` or `fabric-26.1.2.lua`. Each entry resolves JourneyMap's
...
清单选择 `fabric-1.21.8.lua` 或 `fabric-26.1.2.lua`。每个入口通过 Mod classloader 解析
JourneyMap `WaypointFactory`，并从 `minecraft.client_objects` 获取映射后的方块坐标与维度键。
后备入口在未知 Fabric 版本登记
