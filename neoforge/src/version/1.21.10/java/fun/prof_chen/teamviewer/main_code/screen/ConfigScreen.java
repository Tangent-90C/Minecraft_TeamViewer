        ...
        if (control.kind() == ConfigControlKind.TEXT_FIELD) {
            ...
            if (control.labelBounds() != null) {
                ...
                StringWidget labelWidget = new StringWidget(label.x(), label.y(), label.width(), label.height(),
                        toComponent(control.label()), font).setColor(control.color());
            ...
            Component text = toComponent(control.label());
            int textWidth = Math.min(bounds.width(), font.width(text));
            int textX = control.alignment() == ConfigControlView.TextAlignment.CENTER
                    ? bounds.x() + (bounds.width() - textWidth) / 2
                    : bounds.x();
            StringWidget label = new StringWidget(textX, bounds.y(), textWidth, bounds.height(), text, font);
