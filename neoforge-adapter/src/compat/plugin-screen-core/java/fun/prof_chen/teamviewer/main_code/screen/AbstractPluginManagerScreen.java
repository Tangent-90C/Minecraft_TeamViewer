...
abstract class AbstractPluginManagerScreen extends Screen {
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float delta) {
        ...
        if (tooltip != null) showTooltip(graphics, toComponent(tooltip), mouseX, mouseY);
    }

    protected abstract void showTooltip(
            GuiGraphics graphics, Component tooltip, int mouseX, int mouseY);

    protected final boolean finishMouseClicked(
            double mouseX, double mouseY, int button,
            ConfigControlId focusedBefore, boolean handledByField) {
    ...
    protected final boolean finishKeyPressed(
            int keyCode, ConfigControlId focusedBefore, boolean handledByField) {
        if (focusedBefore != null && !focusedBefore.equals(focusedField())) {
            controller.commitTextSettings();
        }
        if (handledByField) return true;
    ...
    protected final ConfigControlId focusedField() {
    ...
    private static Component toComponent(UiText value) {
        ...
        MutableComponent component = value.translationKey() == null
                ...
                : Component.translatable(value.translationKey(),
                        value.arguments().stream().map(AbstractPluginManagerScreen::toComponent).toArray());
