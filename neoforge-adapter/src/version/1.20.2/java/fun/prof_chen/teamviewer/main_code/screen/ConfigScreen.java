...
public final class ConfigScreen extends Screen {
    ...
    private void addControl(ConfigControlView control) {
        ...
        if (control.kind() == ConfigControlKind.TEXT_FIELD) {
            ...
            Checkbox checkbox = new Checkbox(bounds.x(), bounds.y(), bounds.width(), bounds.height(),
                    toComponent(control.label()), control.checked()) {
                @Override
                public void onPress() {
                    super.onPress();
                    session.setChecked(control.id(), selected());
                    refreshDynamicControls();
                }
            };
            if (control.tooltip() != null) {
                checkbox.setTooltip(Tooltip.create(toComponent(control.tooltip())));
            }
            widget = checkbox;
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float delta) {
        ...
        for (ConfigControlView control : page.controls()) {
            if (control.tooltip() != null && control.kind() == ConfigControlKind.TEXT
                ...
                graphics.renderTooltip(font, toComponent(control.tooltip()), mouseX, mouseY);
                break;
            }
        }
    }
