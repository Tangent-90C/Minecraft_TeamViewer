...
/** Dense JsMacros-inspired plugin manager host for the NeoForge 26.1 source family. */
public final class PluginManagerScreen extends Screen {
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
        ...
        boolean handledByField = event.button() == 0 && super.mouseClicked(event, doubleClick);
        if (focusedBefore != null && !focusedBefore.equals(focusedField())) {
            controller.commitTextSettings();
        }
        if (handledByField) return true;
        if (event.button() != 0) return false;
        ConfigControlId id = PluginManagerPainter.actionAt(view, (int) event.x(), (int) event.y());
    ...
    public boolean keyPressed(KeyEvent event) {
        ConfigControlId focusedBefore = focusedField();
        boolean handledByField = super.keyPressed(event);
        if (focusedBefore != null && !focusedBefore.equals(focusedField())) {
            controller.commitTextSettings();
        }
        if (handledByField) return true;
        if (view.dialog() == null
            ...
            controller.commitTextSettings();
            controller.moveSelection(event.key() == GLFW.GLFW_KEY_UP ? -1 : 1);
            refresh();
            return true;
        }
        return false;
    ...
    private void refresh() {
        MinecraftClientUiCompat.setScreen(minecraft, new PluginManagerScreen(parent, controller));
    }
    ...
    private final class Canvas implements PluginManagerPainter.Canvas {
        ...
        private Canvas(GuiGraphicsExtractor graphics) {
        ...
        public void text(UiText text, UiRect bounds, int color, boolean shadow) {
            ...
            graphics.text(font, value, bounds.x(), bounds.y(), color, shadow);
        }
        ...
        public void centered(UiText text, int centerX, int y, int color) {
            Component component = toComponent(text);
            graphics.text(font, component, centerX - font.width(component) / 2, y, color, false);
        }
